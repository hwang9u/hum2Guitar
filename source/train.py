...
hummingset = Hum2GuitarSet(config.guitar_dir, input_type='humming', stft_kwargs=stft_kwargs, mel_kwargs=mel_kwargs)
...
global_trainer.create_scheduler()
global_trainer.train(h_s=h_s, x_s=x_s, FM_LAMBDA=FM_LAMBDA)


local_trainer =  Trainer(config, train_dloader=train_dloader, stage="local", stft_kwargs=stft_kwargs, mel_kwargs=mel_kwargs, device = device)
local_trainer.create_optimizer(lr = lr, beta1=beta1, beta2=beta2)
local_trainer.create_scheduler()
