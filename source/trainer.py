...
class Trainer:
    def __init__(self, config, train_dloader,  stft_kwargs, mel_kwargs, stage="global", device= "cpu", lr_decay_epochs=100):
        self.device = device
        self.stage=stage
        self.config = config
        ...
        self.n_epochs = config.n_global_epochs if self.stage else config.n_fixed_global_epochs + config.n_joint_epochs
        ...
        if config.pretrained_global is not None:
            printlog(f'\n[STAGE 1] Load Pre-trained Global Generator/Discriminator from {self.config.pretrained_global}', self.LOG_PATH)
            self.load_checkpoint(config.pretrained_global)
        if stage == "local":
            net_LE = LocalEnhancer(input_dim=1, global_generator=None)
            net_LE.apply(init_weights)
            net_LE.global_generator = nn.Sequential(*[getattr(self.net_G, k) for k in ['down', 'res', 'up']])
            self.net_G = net_LE

            if config.pretrained_local is not None:
                printlog(f'\n[STAGE 2] Load Pre-trained Local Enhancer/Discriminator from {config.pretrained_local}', self.LOG_PATH)
                self.load_checkpoint(config.pretrained_local)
    ...
    def create_scheduler(self):
        self.scheduler_D = torch.optim.lr_scheduler.LambdaLR(self.optimizer_D, lambda epoch: lr_lambda(epoch, n_epochs=self.n_epochs, decay_epoch=self.lr_decay_epochs), verbose = True)
        self.scheduler_G = torch.optim.lr_scheduler.LambdaLR(self.optimizer_G, lambda epoch: lr_lambda(epoch, n_epochs=self.n_epochs, decay_epoch=self.lr_decay_epochs), verbose = True)
    ...
    def train(self, h_s, x_s, var= 0., FM_LAMBDA=10):
        for e in range(self.epoch_, self.n_epochs):
            ...
            if self.stage =="global":
                if e == self.epoch_:
                    printlog(f'\n[STAGE 1] Train Global Generator for {self.config.n_global_epochs} epochs\n', self.LOG_PATH)
            
            elif self.stage=="local":
                printlog(f'\n[STAGE 2] Train Local Enhancer for {self.n_epochs} epochs', self.LOG_PATH)
                if (self.config.n_fixed_global_epochs> 0) & (e == self.epoch_):
                    printlog(f"--> Train Only Local Enhancer for {self.n_fixed_global_epochs} epochs", self.LOG_PATH)
                    trainable_global(self.net_G, grad=False) 
                elif e == (self.epoch_ + self.config.n_fixed_global_epochs):
                    printlog(f"--> Train Global Generator and Local Enhancer for {self.n_joint_epochs} epochs", self.LOG_PATH)
            ...
            with tqdm(self.train_dloader, unit='batch') as tepoch:
                for b, (h, x, _) in enumerate(tepoch):       
                    ...
                    # D(x)
                    y_real = self.net_D(x_real + noise, h)
                    ...
                    if b % self.config.plot_interval == 0:
                            ...
                            with torch.no_grad():
                                ...
                                fig.tight_layout()
                                fig.savefig(f'{self.GUITAR_SAMPLE_SAVE_DIR}/le_sample_{str(plot_ind).zfill(4)}.jpg')
                                plt.close(fig)
                                ...
                                fig.suptitle(f'Epoch {e+1}')
                                fig.tight_layout()
                                fig.savefig(f'{self.HUMMING_SAMPLE_SAVE_DIR}/le_sample_{str(plot_ind).zfill(4)}.jpg')
                                plt.close(fig)
                                plot_ind += 1   
